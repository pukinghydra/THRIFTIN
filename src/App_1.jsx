    ...
    <div style={{ padding:"16px 16px 0" }}>
      ...
      {adminMode && adminUser && (
        <div style={{ marginTop:28, marginBottom:24 }}>
          ...
          <ExportPanel users={users} shifts={shifts} yr={yr} mo={mo} />
        </div>
      )}
...
function ExportPanel({ users, shifts, yr, mo }) {
  ...
  const buildText = () => {
    let out = "Thriftin' hours \u2014 " + MO_LONG[mo] + " " + yr + "\n";
    users.filter(u => selected.includes(u.id)).forEach(u => {
      const rows = monthDays.map(ds => ({ ds, sh: shifts.find(s => s.user_id === u.id && s.date === ds) }));
      out += "\n" + u.name + (u.tracks_hours ? " (hourly)" : "") + "\n";
      if (u.tracks_hours) {
        const worked = rows.filter(x => x.sh && x.sh.status === "worked");
        worked.forEach(x => { out += "  " + fmtDay(x.ds) + "   " + x.sh.hours + "h\n"; });
        const total = worked.reduce((s, x) => s + hoursNum(x.sh.hours), 0);
        out += "  Total: " + total + "h over " + worked.length + " day" + (worked.length === 1 ? "" : "s") + "\n";
      } else {
        const days = rows.filter(x => x.sh && x.sh.status !== "off");
        days.forEach(x => { out += "  " + fmtDay(x.ds) + "\n"; });
        out += "  Days worked: " + days.length + "\n";
      }
    });
    return out;
  };
  ...
  return (
    <div style={{ marginTop:30 }}>
      <div style={{ fontSize:10, fontWeight:800, color:"#c33", letterSpacing:1.2, marginBottom:12 }}>ADMIN {"\u2014"} EXPORT {"\u00b7"} {MO_LONG[mo]} {yr}</div>
      <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginBottom:8 }}>
        {users.map(u => (
          <button key={u.id} onClick={() => toggle(u.id)} style={{ ...S.chip(selected.includes(u.id), u.color), padding:"8px 14px", fontSize:13 }}>{u.name}</button>
        ))}
      </div>
      <div style={{ display:"flex", gap:8, marginBottom:12 }}>
        <button onClick={() => setSelected(users.map(u => u.id))} style={{ padding:"6px 12px", background:CARD, border:"1px solid "+BORDER, borderRadius:8, fontSize:12, fontWeight:600, color:"#555", cursor:"pointer", fontFamily:"inherit" }}>Everyone</button>
        <button onClick={() => setSelected([])} style={{ padding:"6px 12px", background:CARD, border:"1px solid "+BORDER, borderRadius:8, fontSize:12, fontWeight:600, color:"#555", cursor:"pointer", fontFamily:"inherit" }}>Clear</button>
      </div>
      <textarea readOnly value={text} rows={10} onFocus={e => e.target.select()}
        style={{ width:"100%", boxSizing:"border-box", border:"2px solid "+BORDER, borderRadius:12, padding:"12px 14px", fontSize:13, lineHeight:1.5, fontFamily:"'SF Mono', Menlo, monospace", color:DARK, background:CARD, resize:"vertical", whiteSpace:"pre", overflowX:"auto", outline:"none" }} />
      <button onClick={copy} style={{ ...S.btn(selected.length > 0), marginTop:10 }}>{copied ? "Copied \u2713" : "Copy text"}</button>
      <div style={{ fontSize:12, color:MUTED, marginTop:8, lineHeight:1.4 }}>Exports the month shown above {"\u2014"} use the {"\u2039 \u203a"} arrows to change month. Hourly = confirmed hours only; owners = days on the schedule.</div>
    </div>
  );
}
